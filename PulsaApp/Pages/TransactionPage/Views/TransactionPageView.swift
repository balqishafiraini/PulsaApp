...
class TransactionPageView: UIView {
    ...
    var appliedVoucher: VoucherItems? { // Properti untuk menyimpan voucher yang diterapkan
            didSet {
                updateProductDetails()
            }
        }
    
    private var discountAmount: Double = 1000.0
    private var paymentDueInDays: Int = 30
    private var voucherName: String? = "Voucher untukmu" // This property seems unused, consider removing if not needed
    ...
    private lazy var voucherLabel: UILabel = {
        let label = UILabel()
        label.text = "Voucher untukmu" // This text can be dynamic based on actual voucher
    ...
    func updateVoucherDisplay() {
            if let voucher = appliedVoucher {
                voucherLabel.text = voucher.name
                voucherButton.setTitle("Ganti", for: .normal)
            } else {
    ...
    private func setupViews() {
        ...
        let voucherSectionStack = UIStackView(arrangedSubviews: [voucherSectionLabel, voucherContainerView])
        voucherSectionStack.axis = .vertical
        voucherSectionStack.spacing = 10

        addSubview(voucherSectionStack)
        voucherSectionStack.anchor(top: paymentDetailsContainerView.bottomAnchor, left: leftAnchor, right: rightAnchor, paddingTop: 20, paddingLeft: 16, paddingRight: 16)
        ...
        voucherIcon.anchor(left: voucherContainerView.leftAnchor, paddingLeft: 12, width: 24, height: 24)
        ...
        voucherButton.anchor(right: voucherContainerView.rightAnchor, paddingRight: 12)
        voucherButton.centerY(inView: voucherContainerView)
        voucherButton.widthAnchor.constraint(equalToConstant: 60).isActive = true
        voucherContainerView.heightAnchor.constraint(equalToConstant: 50).isActive = true


        addSubview(pinKredivoContainerView)
        pinKredivoContainerView.anchor(top: voucherSectionStack.bottomAnchor, left: leftAnchor, right: rightAnchor,
                                       paddingTop: 20)
        ...
        showPinButton.anchor(right: pinTextField.rightAnchor, paddingRight: 12, width: 24, height: 24)
    ...
    func updateProductDetails() {
            ...
            if let voucher = appliedVoucher {
                // Hitung diskon berdasarkan tipe voucher (persentase atau nominal)
    ...
    private func formatPhoneNumberForDisplay(_ number: String) -> String {
        ...
        // Insert spacing for readability (e.g. 6283 7370 1234)
    ...
    @objc private func termsAndConditionsTapped() {
        ...
        // You can add a delegate call here too if the VC needs to handle this
    }
    ...
    @objc private func payButtonTapped() {
        ...
        guard let product = selectedProduct, let phoneNumber = phoneNumber else {
            ...
            // Potentially show an alert for missing data
            return
        }
